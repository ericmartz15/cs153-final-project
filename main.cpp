...
void putImageInWindow(GWindow& gw, GCanvas& img, string title) {
    gw.setTitle(title);
    gw.setVisible(true);
    gw.requestFocus();
    gw.add(&img, 0, 0);
...
void loadImages() {
    string filename = chooseFilenameDialog("Please choose a .jpg file", "res/jpegs");
    GCanvas img;
    loadJpeg(img, filename);
    GWindow originalWindow;
    originalWindow.setLocation(0, 250);
    size_t lastSlashIndex = filename.rfind("/");
    if (lastSlashIndex != string::npos) {
        filename = filename.substr(lastSlashIndex + 1);
    }
    putImageInWindow(originalWindow, img, filename);
}
